...
var Designers = require('../models/designerList');
...
//     Designers.find({}, function(err,dish){
//         if(err) throw err;
//         res.json(dish);
//
//     });
...
//     Designers.create(req.body, function(err,dish) {
//         if(err) throw err;
...
.delete(Verify.verifyOrdinaryUser,function(req,res,next) {
    Designers.remove({}, function(err, resp){
        if(err) return next(err);
        res.json(resp);
    });
});
...
.get(function(req,res,next){
    Designers.findById(req.params.dishId, function(err, dish) {
        if(err) return next(err);
        res.json(dish);
    });
})
...
.put(function(req,res,next) {
    Designers.findByIdAndUpdate(req.params.dishId, {
        $set:req.body
    }, {
        new:true
    },function (err,dish) {
...
.delete(function(req,res,next) {
    Designers.findByIdAndRemove(req.params.dishId, function(err, resp){
        if(err) throw err;
        res.json(resp);
    });
});
